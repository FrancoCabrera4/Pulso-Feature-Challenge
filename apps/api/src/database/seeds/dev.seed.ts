...
async function seedDev(dataSource: DataSource) {
  ...
  const recipeToNutritionalCategoryRepo = dataSource.getRepository(
  ...
  // Create tags
  const tagConCarne = await tagRepo.save(
    tagRepo.create({
      description: 'Con carne',
    }),
  );

  const tagPlatoPrincipal = await tagRepo.save(
    tagRepo.create({
      description: 'Plato principal',
    }),
  );

  const tagBajaEnSodio = await tagRepo.save(
    tagRepo.create({
  ...
  const catCalories = await nutritionalCategoryRepo.save(
    nutritionalCategoryRepo.create({
      description: 'Calorías',
    }),
  );

  const catProteins = await nutritionalCategoryRepo.save(
    nutritionalCategoryRepo.create({
      description: 'Proteínas',
    }),
  );

  const catCarbs = await nutritionalCategoryRepo.save(
    nutritionalCategoryRepo.create({
      description: 'Carbohidratos',
    }),
  );

  const catFats = await nutritionalCategoryRepo.save(
    nutritionalCategoryRepo.create({
      description: 'Grasas',
    }),
  );

  const catFiber = await nutritionalCategoryRepo.save(
    nutritionalCategoryRepo.create({
      description: 'Fibra',
    }),
  );
  ...
  const recipe = await recipeRepo.save(
    recipeRepo.create({
      ...
      description:
        'Esta receta presenta una ensalada estilo diosa verde que combina kale lacinato, pimientos y tomates, acompañada de pollo marinado en una cremosa mezcla de suero de leche y yogur griego.',
      portions: 4,
      preparationTimeMinutes: 288, // 4.8 hours = 288 minutes
      imageUrl:
        'https://pulso-feature-bucket.s3.sa-east-1.amazonaws.com/bowls_vegetales_pollo_diosa.png',
  ...
  const ingredients = [
    ...
    {
      order: 3,
      description: '15 g de perejil fresco de hoja plana, ligeramente empacado',
    },
    { order: 4, description: '1 cucharada de cebollino fresco picado' },
    {
      order: 5,
      description:
        '1 cucharada de jugo de limón más 2 cucharaditas de ralladura de limón, divididas',
    },
    ...
    {
      order: 11,
      description: '4 tazas de kale dinosaurio, cortado en tiras finas',
    },
    { order: 12, description: '1/2 pepino mediano, en rodajas' },
    {
      order: 13,
      description:
        '2 pimientos, preferiblemente 1 rojo y 1 naranja o amarillo, picados',
    },
    {
      order: 14,
      description:
        '1 pinta de tomates cherry, partidos por la mitad si son grandes',
    },
  ...
  const procedureSteps = [
    ...
    },
    {
      order: 4,
      description:
        'Agregar el kale y 1/4 taza del aderezo reservado a la quinua; revolver para combinar. Dividir la mezcla de quinua en 4 tazones. Cubrir con pepino, pimientos, tomates y pollo. Rociar el 1/4 taza de aderezo restante sobre los tazones. Cubrir con aguacate, queso y almendras.',
    },
  ];

  for (const stepData of procedureSteps) {
    await procedureStepRepo.save(
      procedureStepRepo.create({
        ...stepData,
        recipe: recipe,
      }),
    );
  ...
  for (const info of nutritionalInfo) {
    await recipeToNutritionalCategoryRepo.save(
      recipeToNutritionalCategoryRepo.create({
        recipe: recipe,
  ...
  const tagSinGluten = await tagRepo.save(
    tagRepo.create({
      description: 'Sin gluten',
    }),
  );

  const tagSinLactosa = await tagRepo.save(
    tagRepo.create({
      description: 'Sin lactosa',
    }),
  );

  const tagSinFrutosSecos = await tagRepo.save(
    tagRepo.create({
      description: 'Sin frutos secos',
    }),
  );

  const tagAltaEnProteinas = await tagRepo.save(
    tagRepo.create({
      description: 'Alta en proteínas',
    }),
  );

  const tagBajaEnCarbohidratos = await tagRepo.save(
    tagRepo.create({
  ...
  const recipe2 = await recipeRepo.save(
    recipeRepo.create({
      ...
      imageUrl:
        'https://pulso-feature-bucket.s3.sa-east-1.amazonaws.com/pechugas_pollo_asadas_horno.png',
      tags: [
        tagConCarne,
        tagSinGluten,
        tagSinLactosa,
  ...
  const ingredients2 = [
    ...
    { order: 4, description: '5 ml de sirope de agave' },
    {
      order: 5,
      description:
        '2 pechugas de pollo sin hueso y sin piel (aproximadamente 225 g cada una)',
    },
  ];
  ...
  const procedureSteps2 = [
    ...
    },
    {
      order: 6,
      description:
        'Retirar del horno y dejar reposar 5-10 minutos antes de cortar y servir.',
    },
  ];
  ...
  const tagAltaEnFibra = await tagRepo.save(
    tagRepo.create({
  ...
  // Create third recipe
  const recipe3 = await recipeRepo.save(
    recipeRepo.create({
      title: 'Ensalada César de coles de Bruselas',
      description:
        'La ensalada César de coles de Bruselas combina el sabor asado de las coles con el toque característico de salsa Worcestershire y queso parmesano.',
      portions: 4,
      preparationTimeMinutes: 25,
      imageUrl:
        'https://pulso-feature-bucket.s3.sa-east-1.amazonaws.com/ensalada_cesar_coles_bruselas.png',
  ...
  const ingredients3 = [
    { order: 1, description: '2 cucharadas de aceite de oliva' },
    {
      order: 2,
      description:
        '450 gr de coles de Bruselas medianas, recortadas y cortadas por la mitad a lo largo',
    },
    ...
    {
      order: 10,
      description: '4 cucharadas de queso parmesano rallado, dividido',
    },
    {
      order: 11,
      description: '2 tazas de corazones de lechuga romana picados',
    },
    {
      order: 12,
      description:
        '1/2 taza de crutones sazonados al estilo César, picados groseramente',
    },
  ];
  ...
  const procedureSteps3 = [
    ...
    },
    {
      order: 2,
      description:
        'Calentar aceite de oliva en una sartén grande a fuego medio-alto hasta que brille. Agregar las coles de Bruselas con el lado cortado hacia abajo; cocinar sin mover hasta que estén doradas por debajo, aproximadamente 4 a 5 minutos.',
    },
    {
      order: 3,
      description:
        'Dar vuelta las coles de Bruselas. Transferir al horno; asar hasta que estén doradas en los bordes y tiernas-crujientes, de 6 a 8 minutos. Dejar enfriar ligeramente durante aproximadamente 5 minutos.',
    },
    {
      order: 4,
      description:
        'Mientras tanto, batir la ralladura de limón, jugo de limón, mayonesa, ajo, vinagre, Worcestershire, pimienta y 3 cucharadas de parmesano en un tazón grande hasta que quede suave. Agregar las coles de Bruselas, la lechuga romana y los crutones; mezclar hasta quedar bien cubiertos.',
    },
    {
      order: 5,
      description:
        'Distribuir en 4 platos y espolvorear uniformemente con la cucharada restante de parmesano.',
    },
  ];
  ...
  const tagPostre = await tagRepo.save(
    tagRepo.create({
      description: 'Postre',
    }),
  );

  const tagBajaEnGrasas = await tagRepo.save(
    tagRepo.create({
      description: 'Baja en grasas',
    }),
  );

  const tagBajaEnCalorias = await tagRepo.save(
    tagRepo.create({
  ...
  const recipe4 = await recipeRepo.save(
    recipeRepo.create({
      ...
      imageUrl:
        'https://pulso-feature-bucket.s3.sa-east-1.amazonaws.com/helado_banana_arandanos.png',
      tags: [
        tagSinGluten,
        tagSinLactosa,
        tagPostre,
  ...
  const procedureSteps4 = [
    {
      order: 1,
      description:
        'Licuá o mixeá las bananas y los arándanos congelados. Pasá la mezcla a los recipientes para servir el helado.',
    },
    {
      order: 2,
